...
class BScaleController(implicit p: Parameters) extends CuteModule{
    ...
    val bankId = RegInit(0.U(log2Ceil(BScaleNBanks).W))
    ...
    val ScarchPadDataHoldReg = RegInit(VecInit(Seq.fill(BScaleNBanks)(0.U(ScaleWidth.W)))) //保存ScarchPad的数据，当发生MTE的NACK时，可以不需要重新从ScarchPad读数
    ...
    when(state === s_mm_task){
        when(calculate_state === s_cal_init){
            ...
            next_addr := (N_Iterator * Matrix_N.U * K_IteratorMax + K_Iterator) * ScaleVecWidth(dataType) * 8.U / outsideDataWidth.U 
            bankId := (N_Iterator * Matrix_N.U * K_IteratorMax + K_Iterator) * ScaleVecWidth(dataType) * 8.U / ScaleWidth.U % BScaleNBanks.U
