...
class CUTETile(outer: RoCC2CUTE) extends LazyRoCCModuleImp(outer) with CUTEImplParameters {
    ...
    when(io.cmd.fire && io.cmd.bits.inst.opcode === "h0B".U && io.cmd.bits.inst.funct === YGJKInstConfigs.QueryAcceleratorBusy.funct.U){ //查询加速器是否在运行
      rd_data := ac_busy
    }.elsewhen(io.cmd.fire && io.cmd.bits.inst.opcode === "h0B".U && io.cmd.bits.inst.funct === YGJKInstConfigs.QueryRuntime.funct.U){ //查询加速器运行时间
      rd_data := count
    }.elsewhen(io.cmd.fire && io.cmd.bits.inst.opcode === "h0B".U && io.cmd.bits.inst.funct === YGJKInstConfigs.QueryMemReadCount.funct.U){ //查询加速器对外访存读次数
      rd_data := memNum_r
    }.elsewhen(io.cmd.fire && io.cmd.bits.inst.opcode === "h0B".U && io.cmd.bits.inst.funct === YGJKInstConfigs.QueryMemWriteCount.funct.U){ //查询加速器对外访存写次数
      rd_data := memNum_w
