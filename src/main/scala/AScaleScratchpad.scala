...
class AScaleScratchpad(implicit p: Parameters) extends CuteModule{
    ...
    val write_go = io.ScaleScratchpadIO.FromScaleLoader.BankAddr.zip(io.ScaleScratchpadIO.FromScaleLoader.Data).map{case (a,b) => a.valid && b.valid}
    val read_go = io.ScaleScratchpadIO.FromScaleController.BankAddr.valid && !io.ScaleScratchpadIO.FromScaleLoader.BankAddr.map(_.valid).reduce(_||_)  && !write_go.reduce(_||_) //写优先～
    //为输入信号赋ready
    ...
    val sram_banks = (0 until AScaleNBanks) map { i =>
        ...
        //写数据
        val s0_bank_write_addr = ScaleLoaderBankAddr(i).bits
        val s0_bank_write_data = ScaleLoaderData(i).bits
        val s0_bank_write_valid = ScaleLoaderBankAddr(i).valid && ScaleLoaderData(i).valid
        ...
        val Bank_addr = Mux(read_go, ScaleControllerBankAddr(i), ScaleLoaderBankAddr(i).bits)
        val Bank_wdata = ScaleLoaderData(i).bits
