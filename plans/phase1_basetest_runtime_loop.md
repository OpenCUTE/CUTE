...
- `HWConfig` 能生成 headers 并输出到 `build/hwconfigs/<name>/generated_headers/`。
...
build/
├── hwconfigs/cute2tops_scp64_dramsim32/
...
- 头文件 include 路径：`cute-sdk/include/`(稳定头文件) + `build/hwconfigs/<name>/generated_headers/`(配置相关)
...
- `make HWCONFIG_DIR=build/hwconfigs/cute2tops_scp64_dramsim32` 编译成功
...
行为:
  1. 读取 hwconfig.yaml
  2. 提取 cute.chipyard_config
  3. 提取 cute.generated_headers.output_dir
  4. 创建 output_dir
  5. 调用 scripts/generate-headers.sh <chipyard_config> <output_dir>
  6. 计算 header fingerprint (sha256 of all .generated files)
  7. 写 hwconfig.resolved.yaml、capability.json、header_fingerprint.txt
```

### 1.4.2 产物结构

```text
build/hwconfigs/cute2tops_scp64_dramsim32/
├── hwconfig.resolved.yaml     # hwconfig.yaml 的完整拷贝（含默认值填充）
...
│   ├── capability.json
│   ├── header_fingerprint.txt
│   └── generated_headers/ → ../../../hwconfigs/cute2tops_scp64_dramsim32/generated_headers/
├── test/
